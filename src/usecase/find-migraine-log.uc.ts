...
export class FindMigraineLogUc implements UseCaseInterface<
  ...
  MigraineLogEntryOutputDto | null
> {
  ...
  execute = async (input: FindMigraineLogInputDto): Promise<MigraineLogEntryOutputDto | null> => {
    try {
      ...
      const entity = await this.migraineLogRepository
        ...
        .getOne();
      return entity === null ? null : toMigraineLogEntry(entity);
    } catch (error) {
